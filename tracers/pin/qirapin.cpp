...
#ifndef TARGET_WINDOWS#include <fcntl.h>
...
#define perror_exit(s) _perror_exit("qira: "
  s)
...
static int vsnprintf(char * buf, size_t size,
...
static void * mmap_map(MMAPFILE handle, size_t size, size_t offset = 0) {
  ...
  #ifdef TARGET_IA32E
  size_t high_offset = (offset >> 32) & 0xFFFFFFFF ul;
  #else
  size_t high_offset = 0;
  #endif
  void * ret = WINDOWS::MapViewOfFileEx(fileMapping, FILE_MAP_READ | FILE_MAP_WRITE, high_offset, offset & 0xFFFFFFFF ul, size, NULL);
...
#if defined(TARGET_LINUX)#include "strace/syscalls.h"

#if defined(TARGET_IA32E)
#define SYS_READ 0#include "strace/syscallents_64.h"

#else
#define SYS_READ 3#include "strace/syscallents_32.h"

#endif
#elif defined(TARGET_MAC)
#define SYS_READ 3#include "strace/osx_syscalls.h"

#else#include "strace/syscalls.h"
...
static
const ADDRINT WRITEEA_SENTINEL = (sizeof(ADDRINT) > 4) ? (ADDRINT) 0xDEADDEADDEADDEAD ull : (ADDRINT) 0xDEADDEAD ul;
...
class Thread_State {
  ...
  static inline void tls_destruct(void * tls) {
    delete static_cast < Thread_State * > (tls);
  }
...
static void add_big_change(THREADID tid, uint64_t addr,
  const void * data, uint32_t flags, size_t size) {
...
static inline void syscall_tls_destruct(void * tls) {
  delete static_cast < Syscall_TLS * > (tls);
}
...
VOID ThreadFini(THREADID tid,
  const CONTEXT * ctx, INT32 code, VOID * v) {
...
#ifndef TARGET_WINDOWS
VOID ForkBefore(THREADID tid,
  const CONTEXT * ctx, VOID * v) {
  process_state.fork_before(tid);
}
VOID ForkAfterParent(THREADID tid,
  const CONTEXT * ctx, VOID * v) {
  process_state.fork_after_parent(tid);
}
VOID ForkAfterChild(THREADID tid,
  const CONTEXT * ctx, VOID * v) {
